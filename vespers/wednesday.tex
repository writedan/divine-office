...
\skiplines{1}\gabcsnippet{
	(c3) Ex(e)cept(f) (:?) the(hr) Lord(h) <b>build</b>(i) the(hr) house() <sp>*</sp>(:) their(hr) labor() is() but(h) <b>lost</b>(f) who(h) build(i) it.(hr)
}
...
\skiplines{0}\gabcsnippet{
	(c3) Bles(e)sed(f) (:?) are(hr) all() they() who(h) <b>fear</b>(i) the(hr) Lord() <sp>*</sp>(:) (hr) <b>and</b>(f) walk(h) in(i) his(hr) ways.()
}
...
\skiplines{1}\gabcsnippet{
	(c3) Out(e) of(f) (:?) the(hr) depths() have() I() cried() un()to(h) <b>thee</b>,(i) O(hr) Lord() <sp>*</sp>(:) (hr) <b>Lord</b>(g) hear(f) my(hr) voice.()
}
