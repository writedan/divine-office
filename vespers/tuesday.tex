...
\skiplines{1}\gabcsnippet{
	(c4) Un(g)to(h) (:?) thee(jr) I() lift(j) <b>up</b>(k) mine(j) eyes() <sp>*</sp>(:) O(jr) thou() that() dwellest(j) <b>in</b>(i) the(j) hea(h)vens.(gr)
}
...
\skiplines{0}\gabcsnippet{
	(c3) They(e) that(f) (:?) put(hr) their() trust() in() the() Lord() shall() be() even() as() the() mount(h) <b>Zi</b>(i)on(hr) <sp>*</sp>(:) which(hr) may() not() be() removed,() but stand()eth(h) <b>fast</b>(f) for(h)ev(i)er.(hr)
}
