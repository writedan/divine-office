\vesectionp{Introduction}\label{compline:introduction}
	...
	\skiplines{1}\gabcsnippet{(c3) <sp>v</sp> O(f) God,(hv) <sp>+</sp> make(h) speed(iv) to(h) save(g) me.(hv) <sp>r</sp>(::) O(hv) Lord,(h) make(h) haste(h) to(h) help(g) me.(hv)}\pagebreak[3]
...
\vesectionp{Psalmody and Propers}
	...
	\instruct{Once the psalmody has been completed, the lector proclaims the chapter from the capitulary. The congregation responds.}
	\skiplines{1}\gabcsnippet{(c3) <sp>r</sp> Thanks(h) be(h) to(h) God.(h)}

	\instruct{The lector will then intone the responsory and sing its verses.}
...
\vesectionp{Conclusion}\label{compline:conclusion}
	...
	\instruct{All turn back to the \blueribbon and sing the Marian anthem there appointed with its versicle. The minister then says}
	...
	\skiplines{1}\gabcsnippet{<sp>r</sp> A(gh)men.(h)}\pagebreak[3]
	
	\skiplines{1}\gabcsnippet{
		(c3) <sp>v</sp> May(h) the(h) di(h)vine(h) a(h)ssis(h)tance(h) <sp>+</sp> remain(h) with(h) us(h) al(h)ways,(h) and(h) with(h) our(h) ab(h)sent(h) bre(h)thren.(d)
		<sp>r</sp>(::) A(gh)men.(h)
	}
