\input{conf}

\begin{document}
	\newgeometry{margin=0pt}
	\begin{titlepage}
		\vspace*{\fill}
		
		\begin{center}
			
			\textbf{\Huge\color{red} VESPERAL}
			
			ACCORDING TO THE RITE OF THE
			
			\textbf{\LARGE HOLY ROMAN CHURCH}
			
			\vspace*{\fill}
			
			\textbf{\Large\color{red} VOLUME III}
			
			First Sunday after Epiphany\\Until the First Sunday in Lent
			
		\end{center}
		
		\vspace*{\fill}
	\end{titlepage}
	
	\newgeometry{top=0.25in,bottom=0.5in,inner=0.75in,outer=0.75in,includehead,nofoot}
	\pagestyle{fancy}
	
	\pagenumbering{arabic}
	\gresetinitiallines{0}
	
	\raggedbottom
	
	%\tableofcontents
	
	
	%%% COPY ABOVE %%%
	
	\input{vol3-after-epiphany-calendar}
	
	\vepart{Ordinary}
	
		\vechapter{The Order of Vespers}
		
			\input{vespers/order}
			
		\vechapter{The Order of Compline}
		
			\input{compline/order}
			
	\gresetinitiallines{1}
			
	\vepart{Psalter}
	
		\vechapter{Sunday at Vespers}
		
			\input{vespers/sunday}
		
		\vechapter{Monday at Vespers}
		
			\input{vespers/monday}
		
		\vechapter{Tuesday at Vespers}
		
			\input{vespers/tuesday}
		
		\vechapter{Wednesday at Vespers}
		
			\input{vespers/wednesday}
		
		\vechapter{Thursday at Vespers}
		
			\input{vespers/thursday}
		
		\vechapter{Friday at Vespers}
		
			\input{vespers/friday}
		
		\vechapter{Saturday at Vespers}
		
			\input{vespers/saturday}
		
		\vechapter{Compline}
		
			\input{compline/ordinary}
	
	\vepart{Common Tones}
		
		\input{common/vespers}
			
		\vechapter{Compline}
		
			\input{compline/feria}
	
	\vepart{Temporal}
	
		\vechapter{First Sunday after Epiphany}
		
			\skiplines{-1}\vesectionp{At First Vespers}
			
				\greannotation{\veformat{Ant.}}
				\greannotation{\veformat{viii. i.}}
				\gregorioscore{ant/baptizat_miles_regem.gabc}
				
				\vecomment{Canticle of Mary \enquote{Magnificat}}
				
				\greannotation{\veformat{Cant.}}
				\greannotation{\veformat{viii. i.}}
				\gabcsnippet{
					(c4) My(g) soul(h:?) doth(jr) mag--()ni(j)<b>fy</b>(k) the(jr) Lord() <sp>*</sp>(:) and(jr) my() spirit() hath() rejoiced() in(j) <b>God</b>(i) my(j) Sav(h)ior.(gr)
				}
				
				\input{psalter/magnificat/8}
				
				\repant{ant/baptizat_miles_regem.gabc}
				
				\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
			
			\vesection{At Second Vespers}
			
				\greannotation{\veformat{Ant.}}
				\greannotation{\veformat{viii. i.}}
				\gregorioscore{ant/fontes_aquarum_sanctificati_sunt.gabc}
				
				\vecomment{Canticle of Mary \enquote{Magnificat}}
				
				\greannotation{\veformat{Cant.}}
				\greannotation{\veformat{viii. i.}}
				\gabcsnippet{
					(c4) My(g) soul(h:?) doth(jr) mag--()ni(j)<b>fy</b>(k) the(jr) Lord() <sp>*</sp>(:) and(jr) my() spirit() hath() rejoiced() in(j) <b>God</b>(i) my(j) Sav(h)ior.(gr)
				}
				
				\input{psalter/magnificat/8}
				
				\repant{ant/fontes_aquarum_sanctificati_sunt.gabc}
				
				\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
	
		\vechapter{Second Sunday after Epiphany}
		
			\skiplines{-1}\vesectionp{At First Vespers}
			
%				\instruct{This antiphon is sung for all Sundays at first Vespers beginn.}
				
				\greannotation{\veformat{Ant.}}
				\greannotation{\veformat{i. i.}}
				\skiplines{0.5}\gregorioscore{ant/peccata_mea_domine.gabc}
				
				\vecomment{Canticle of Mary \enquote{Magnificat}}
				
				\greannotation{\veformat{Cant.}}
				\greannotation{\veformat{i. i.}}
				\gabcsnippet{
					(f3) My(h) soul(i:?) doth(jr) magni--()fy(j) <b>the</b>(i) Lord(j) <sp>*</sp>(:) and(jr) my() spirit() hath() rejoiced() in(j) <b>God</b>(i) my(h) Sav(ij)ior.(ir)
				}
			
				\input{psalter/magnificat/1}
				
				\repant{ant/peccata_mea_domine.gabc}
				
				\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
			
			\cleardoublepage\vesection{At Second Vespers}
			
				\greannotation{\veformat{Ant.}}
				\greannotation{\veformat{vi.}}
				\gregorioscore{ant/puer_jesus_proficiebat.gabc}
				
				\vecomment{Canticle of Mary \enquote{Magnificat}}
				
				\greannotation{\veformat{Cant.}}
				\greannotation{\veformat{vi.}}
				\gabcsnippet{
					(cb4) My(f) soul(g:?) doth(hr) magni--()fy(h) <b>the</b>(g) Lord(h) <sp>*</sp>(:) and(hr) my() spirit() hath() rejoiced() in(h) <b>God</b>(f) my(gh) Sav(g)ior.(fr)
				}
				
				\input{psalter/magnificat/6}
				
				\clearpage\skiplines{-1}\repant{ant/puer_jesus_proficiebat.gabc}
				
				\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
		
		\vechapter{Third Sunday after Epiphany}
		
			\greannotation{\veformat{Ant.}}
			\greannotation{\veformat{i. i.}}
			\gregorioscore{ant/deficiente_vino.gabc}
			
			\vecomment{Canticle of Mary \enquote{Magnificat}}
			
			\greannotation{\veformat{Cant.}}
			\greannotation{\veformat{i. i.}}
			\gabcsnippet{
				(cb4) My(f) soul(g:?) doth(hr) magni--()fy(h) <b>the</b>(g) Lord(h) <sp>*</sp>(:) and(hr) my() spirit() hath() rejoiced() in(h) <b>God</b>(g) my(f) Sav(gh)ior.(gr)
			}
			
			\input{psalter/magnificat/1}
			
			\clearpage\skiplines{-1}\repant{ant/deficiente_vino.gabc}
			
			\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
		
		\vechapter{Fourth Sunday after Epiphany}
		
			\greannotation{\veformat{Ant.}}
			\greannotation{\veformat{i. iii.}}
			\gregorioscore{ant/domine_puer_meus.gabc}
			
			\vecomment{Canticle of Mary \enquote{Magnificat}}
			
			\greannotation{\veformat{Cant.}}
			\greannotation{\veformat{i. iii.}}
			\gabcsnippet{
				(c4) My(f) soul(g:?) doth(hr) magni--()fy(h) <b>the</b>(g) Lord(h) <sp>*</sp>(:) and(hr) my() spirit() hath() rejoiced() in(h) <b>God</b>(g) my(f) Sav(gr)ior.(ghg)
			}
			
			\input{psalter/magnificat/1}
			
			\clearpage\skiplines{-1}\repant{ant/domine_puer_meus.gabc}
			
			\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
		
		\vechapter{Fifth Sunday after Epiphany}
		
			\greannotation{\veformat{Ant.}}
			\greannotation{\veformat{viii. i.}}
			\gregorioscore{ant/surgens_jesus_imperavit_ventis.gabc}
			
			\vecomment{Canticle of Mary \enquote{Magnificat}}
			
			\greannotation{\veformat{Cant.}}
			\greannotation{\veformat{viii. i.}}
			\gabcsnippet{
				(c4) My(g) soul(h:?) doth(jr) mag--()ni(j)<b>fy</b>(k) the(jr) Lord() <sp>*</sp>(:) and(jr) my() spirit() hath() rejoiced() in(j) <b>God</b>(i) my(j) Sav(h)ior.(gr)
			}
			
			\input{psalter/magnificat/8}
			
			\clearpage\skiplines{-1}\repant{ant/surgens_jesus_imperavit_ventis.gabc}
			
			\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
		
		\vechapter{Sixth Sunday after Epiphany}
		
			\greannotation{\veformat{Ant.}}
			\greannotation{\veformat{i. i.}}
			\gregorioscore{ant/colligite_primum_zizania.gabc}
			
			\vecomment{Canticle of Mary \enquote{Magnificat}}
			
			\greannotation{\veformat{Cant.}}
			\greannotation{\veformat{i. i.}}
			\gabcsnippet{
				(cb4) My(f) soul(g:?) doth(hr) magni--()fy(h) <b>the</b>(g) Lord(h) <sp>*</sp>(:) and(hr) my() spirit() hath() rejoiced() in(h) <b>God</b>(g) my(f) Sav(gh)ior.(gr)
			}
			
			\input{psalter/magnificat/1}
			
			\clearpage\skiplines{-1}\repant{ant/colligite_primum_zizania.gabc}
			
			\instruct{Turn to the \whiteribbon at the conclusion rite, pp. \pageref{vespers:conclusion}. Cantors, turn to the \blueribbon for the Kyrie.}
		
		\vechapter{Third Week before Lent}
		
			\skiplines{-1}\vesectionp{Sunday at First Vespers}
			
			\cleardoublepage\vesection{Sunday at Second Vespers}
			
			\cleardoublepage\vesection{Monday at Vespers}
			
			\cleardoublepage\vesection{Tuesday at Vespers}
			
			\cleardoublepage\vesection{Wednesday at Vespers}
			
			\cleardoublepage\vesection{Thursday at Vespers}
			
			\cleardoublepage\vesection{Friday at Vespers}
			
		\vechapter{Second Week before Lent}
		
			\skiplines{-1}\vesectionp{Sunday at First Vespers}
		
			\cleardoublepage\vesection{Sunday at Second Vespers}
			
			\cleardoublepage\vesection{Monday at Vespers}
			
			\cleardoublepage\vesection{Tuesday at Vespers}
			
			\cleardoublepage\vesection{Wednesday at Vespers}
			
			\cleardoublepage\vesection{Thursday at Vespers}
			
			\cleardoublepage\vesection{Friday at Vespers}
			
		\vechapter{First Week before Lent}
		
			\skiplines{-1}\vesectionp{Sunday at First Vespers}
			
			\cleardoublepage\vesection{Sunday at Second Vespers}
			
			\cleardoublepage\vesection{Monday at Vespers}
			
			\cleardoublepage\vesection{Tuesday at Vespers}
			
			\cleardoublepage\vesection{Ash Wednesday at Vespers}
			
			\cleardoublepage\vesection{Thursday after Ashes at Vespers}
			
			\cleardoublepage\vesection{Friday after Ashes at Vespers}
	
	\vepart{Sanctoral}
	
		\vechapter{Saint Agnes, Virgin-Martyr}
		
		\vechapter{Conversion of Saint Paul}
		
		\vechapter{Purification of the\\Blessed Virgin Mary}
		
		\vechapter{Saint Agatha, Virgin-Martyr}
		
		\vechapter{Saint Valentine,\\Priest and Martyr}
		
		\vechapter{Saints Perpetua\\and Felicity, Martyrs}
	
\end{document}