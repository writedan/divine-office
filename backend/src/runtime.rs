    ...
    pub fn new() -> Rc<RefCell<Self>> {
            ...
            rt.borrow_mut().define(
                ...
                Value::Native(|_, args| {
                    ...
                    let params = match &args[0] {
                        ...
                        other => {
                            return Err(format!(
                                "Lambda parameters must be a list, got {:?}",
                                other
                            ))
                        }
                    };
            ...
            rt.borrow_mut().define(
                ...
                Value::Native(|env, args| {
                    ...
                    let name = match &args[0] {
                        ...
                        other => {
                            return Err(format!("Variable name must be symbol, got {:?}", other))
                        }
                    };
                ...
                Value::Native(|env, args| {
                    for arg in args {
                        match arg {
                            ...
                            other => {
                                return Err(format!("Arguments must be symbol, got {:?}", other))
                            }
                        }
                    }
            ...
            Value::Native(|env, args| {
                ...
                for arg in args {
                    let val = match Runtime::eval(Rc::clone(&env), &arg) {
                        Ok(val) => val,
                        Err(why) => {
                            Value::Error(format!("Error while evaluating {}: {}", arg, why))
                        }
                    };
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                ...
                match Runtime::eval(Rc::clone(&env), &args[0])? {
                    Value::Symbol(s) => match env.borrow().get(&s) {
                        Some(val) => Ok(val),
                        None => Ok(Value::Nil),
                    },
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                ...
                Ok(Value::Boolean(
                    Runtime::eval(Rc::clone(&env), &args[0])?.is_numeric(),
                ))
            }),
        );
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                ...
                Ok(Value::Title(
                    Runtime::eval(Rc::clone(&env), &args[0])?.to_string(),
                ))
            }),
        );
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                if args.len() != 2 {
                    return Err(
                        "gabc-attr requires two arguments: (gabc-attr <gabc> <attr>)".into(),
                    );
                }
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                ...
                Ok(Value::Boolean(
                    !Runtime::eval(Rc::clone(&env), &args[0])?.is_truthy(),
                ))
            }),
        );
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                if args.len() < 2 {
                    return Err(
                        "match requires at least 2 arguments: (match <value> <case>...)".into(),
                    );
                }
                ...
                for case_expr in &args[1..] {
                    match case_expr {
                        Expr::List(case_parts) if !case_parts.is_empty() => {
                            ...
                            if case_parts.len() < 2 {
                                return Err(
                                    "case requires at least a pattern: (case <pattern> <body>...)"
                                        .into(),
                                );
                            }
                        ...
                        other => {
                            return Err(format!(
                                "Match body must be case expressions, got {:?}",
                                other
                            ))
                        }
                    }
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                ...
                for arg in args {
                    ...
                    out.push(Value::Gabc(GabcFile::new(
                        format!("initial-style: 0;\ncentering-scheme: english;\n%%\n{}", val)
                            .as_str(),
                    )?));
                }
        ...
        rt.borrow_mut().define(
            ...
            Value::Native(|env, args| {
                if args.len() < 1 || args.len() > 2 {
                    return Err(
                        "heading requires at least one argument: (heading <text> [size])".into(),
                    );
                }
                ...
                let level = if args.len() == 1 {
                    ...
                    match Runtime::eval(Rc::clone(&env), &args[1])? {
                        ...
                        other => {
                            return Err(format!(
                                "Second argunebt to heading must be numeric, got {:?}",
                                other
                            ))
                        }
                    }
        ...
        for expr in exprs {
            match Self::eval(Rc::clone(&env), &expr) {
                ...
                Err(why) => env
                    .borrow_mut()
                    .yields
                    .push(Value::Error(format!("Error evaluating {}: {}", expr, why))),
            };
        }
