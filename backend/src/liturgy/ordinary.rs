...
fn vigils(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        "psalter",
        ["vigils", &(iden.day.to_lowercase() + ".lit")]
            .iter()
            .collect(),
    );
...
fn matins(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        "psalter",
        ["matins", &(iden.day.to_lowercase() + ".lit")]
            .iter()
            .collect(),
    );
    ...
    map.insert(
        ...
        [
            ...
            match day {
                Sun => {
                    if iden.season == PostEpiphany(true) || iden.season == PostEpiphany(false) {
                        "eterne-rerum-conditor.lit"
                    } else {
                        "ecce-jam-noctis.lit"
                    }
                }
    ...
    map.insert(
        ...
        if day == Sun {
            ...
            ["commons", "matins", "benedictus", &iden.day.to_lowercase()]
                .iter()
                .collect()
        },
    );
...
fn prime(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        "collect",
        ["commons", "prime", "collect.gabc"].iter().collect(),
    );
...
fn terce(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        ...
        [
            "resp",
            if day == Sun {
                "inclina-cor-meum.gabc"
            } else {
                "sana-animam-meam.gabc"
            },
        ]
        .iter()
        .collect(),
...
fn sext(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        ...
        [
            "resp",
            if day == Sun {
                "in-eternum-domine.gabc"
            } else {
                "benedicam-dominum.gabc"
            },
        ]
        .iter()
        .collect(),
...
fn none(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        ...
        [
            "resp",
            if day == Sun {
                "clamavi-in-toto-corde.gabc"
            } else {
                "redime-me-domine.gabc"
            },
        ]
        .iter()
        .collect(),
...
fn vespers(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        "psalter",
        ["vespers", &(iden.day.to_lowercase() + ".lit")]
            .iter()
            .collect(),
    );
    ...
    map.insert(
        ...
        if day == Sun || day == Sat {
            ...
            ["commons", "vespers", "benedictus", &iden.day.to_lowercase()]
                .iter()
                .collect()
        },
    );
...
fn compline(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
    ...
    map.insert(
        "responsory",
        ["resp", "in-manus-tuas.gabc"].iter().collect(),
    );
    ...
    map.insert(
        "anthem",
        ["commons", "compline", "anthems", "salve-regina.lit"]
            .iter()
            .collect(),
    );
    ...
    map.insert(
        "collect",
        ["commons", "compline", "collect.gabc"].iter().collect(),
    );
