...
fn compline(iden: &Identifier) -> HashMap<&'static str, PathBuf> {
	...
	map.insert("canticle", ["commons", "compline", "canticle", "christmas.lit"].iter().collect());
	map.insert("collect", ["commons", "compline", "collect.lit"].iter().collect());
	map.insert("anthem", ["commons", "compline", "anthems", "alma-redemptoris/christmas.lit"].iter().collect());

	map.insert("kyrie", ["commons", "kyrie", "ii.gabc"].iter().collect());
	map.insert("benedicamus", ["commons", "benedicamus", "ii.gabc"].iter().collect());
